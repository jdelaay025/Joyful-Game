...
public class CamShootHGun : MonoBehaviour 
{
	...
	{
		if (gotEm.gameObject.tag == "HG Ammo" && currentAmmo < maxAmmo) 
		{
			...
			HUDHGAmmo.currentAmmo = clipAmount;
			HUDHGAmmo.clip = clipAmount;

			sound.PlayOneShot(reload);
			//AudioSource.PlayClipAtPoint (reload, transform.position);
		}
		
		if (gotEm.gameObject.tag == "HG Extended Clip" && currentAmmo < extendedAmmo) 
		{
			bulletsUsed = 0;
