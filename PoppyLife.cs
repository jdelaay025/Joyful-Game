...
public class PoppyLife : MonoBehaviour 
{
	public int shots = 0;
	...
	void Start () 
	{
		...
		rend = modelToChange.GetComponent<Renderer> ();
	...
	void Update () 
	{
		...
		if(shots >= hitPoints && gameObject.tag == "Poppy" && !dead)
		{
			dead = true;

			//explosion.Play();
			Instantiate(bob, transform.position + new Vector3(0, 50, 0), transform.rotation);
		...
		if(shots >= hitPoints && gameObject.tag == "Bobby" && !dead)
		{
			dead = true;
